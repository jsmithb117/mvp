...
var Options = (props) => {
  ...
  useEffect(() => {
    ...
    props.restaurantData.forEach((restaurant) => { //restaurantData = [{id: 1, name: 'name', type: 'Sushi', location: 89523, price: 1}...]
      ...
      if (!prices.includes(restaurant.price)) {
  ...
  if (props.isLoggedIn) {
    return (
      <form className="options">
        ...
        <select value={typeSelected} onChange={handleTypesSelectChange} className="option-type" size="4">
        ...
        <select value={locationSelected} onChange={handleLocationsSelectChange} className="option-location">
          {locations.map((location) => {
            return <OptionLocation location={location} key={location} />
          })}
        </select>

        <select value={priceSelected} onChange={handlePriceSelectChange} className="option-price">
          {prices.map((price) => {
